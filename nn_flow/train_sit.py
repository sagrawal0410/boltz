...
from utils.misc import InfiniteSampler, AdaWeighting
...
def train_sit(
    ...
    n_gen_per_step=1,
    job_name="",  # <-- add job_name argument
):
    ...
    ema_slow = EMA(base_model, decay_schedule(1))
    ...
    def load(run_id, epoch, continue_training):
        ...
        info_y = ema_model.model.load_state_dict(loaded["ema_model"] if isinstance(loaded["ema_model"], dict) else loaded["ema_model"].state_dict(), strict=False)
    ...
    def empty_class_labels(bsz):
        return torch.ones(bsz, device=device, dtype=torch.int64) * 1000
    ...
    def samples_to_inputs(samples):
        ...
        labels_dropped = torch.where(torch.rand(labels.shape[0], device=device) < drop_class, empty_class_labels(labels.shape[0]), labels)
        samples = imgs

        samples = preprocess_fn(samples, labels_dropped)
        samples = samples.to(device)
        args = [samples, labels_dropped]
        kwargs = {}
        return args, kwargs
    ...
    for steps in pbar:
        ...
        # FID gen
        if eval_dataset is not None and (steps % eval_gen_per_step == 0 or steps == n_steps - 1): # and steps != 0:
            with torch.inference_mode():
                ...
                if steps != 0:
                    eval_fid(gen_ema, eval_dataset, logger, log_prefix="gen_ema", dataset=dataset_name)
                    eval_fid(gen_model, eval_dataset, logger, log_prefix="gen_model", dataset=dataset_name)
                    # eval_fid(config, gen_slow, eval_dataset, logger, log_prefix="gen_slow", dataset=dataset_name)
                    
                    if steps % (10 * eval_gen_per_step) == 0:
                        eval_fid(gen_ema, eval_dataset, logger, log_prefix="gen_ema", dataset=dataset_name, total_samples=50000, gpu_batch_size=64)
        ...
        if is_main_process and (steps % save_per_step == 0 or steps == n_steps - 1) and steps != 0:
            ...
            save_ckpt(job_name, steps, {"model": model_copy, "ema_model": model_2})
...
def get_args_parser():
    ...
    parser.add_argument('--job_name', type=str, default="", help='Job name for wandb.')
    return parser
...
if __name__ == "__main__":
    ...
    main(args)
# python -m train_sit --config configs/sit_configs/sit_B_1_192.yaml --job_name sit_B_1_192
...
cd "$exp_folder"
port=$((29500 + RANDOM % 1000))  # Random port between 29500-30499
torchrun \
--nnodes=1 \
--nproc-per-node=8 \
--master_port=$port \
...
# Example usage:
# ./run.sh --name=my_experiment --device=0 train
# or just:
# bash ./run.sh -n ar_cd3 -d 0
# bash ./run.sh --slurm --nodes=2 --gpus-per-node=8 -n ar_attn_feats
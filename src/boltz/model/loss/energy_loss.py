...
def denoiser_energy_loss(
    denoised_atom_coords: torch.Tensor,  # [B*multiplicity, N_atoms, 3]
    ground_truth_coords: torch.Tensor,   # [B*multiplicity, N_atoms, 3]
    sigmas: torch.Tensor,                # [B*multiplicity]
    align_weights: torch.Tensor,         # [B*multiplicity, N_atoms]
    resolved_atom_mask: torch.Tensor,    # [B*multiplicity, N_atoms]
    multiplicity: int,
    old_denoised_coords: Optional[torch.Tensor] = None,  # [B*multiplicity, N_atoms, 3] for old_gen
    kernel_type: str = "attn_new",
    aggregate_weights_method: str = "mean",  # "mean" or "sum_normalized"
    **energy_loss_kwargs,  # R_list, target_ratio, n_sinkhorn_steps, etc.
) -> tuple[torch.Tensor, dict]:
    """
    Wrapper function to adapt energy loss to boltz's coordinate format.
    
    Parameters
    ----------
    denoised_atom_coords : torch.Tensor
        Predicted denoised coordinates, shape [B*multiplicity, N_atoms, 3]
    ground_truth_coords : torch.Tensor
        Ground truth aligned coordinates, shape [B*multiplicity, N_atoms, 3]
    sigmas : torch.Tensor
        Noise levels, shape [B*multiplicity]
    align_weights : torch.Tensor
        Per-atom alignment weights, shape [B*multiplicity, N_atoms]
    resolved_atom_mask : torch.Tensor
        Per-atom mask for resolved atoms, shape [B*multiplicity, N_atoms]
    multiplicity : int
        Number of denoised samples per batch item
    old_denoised_coords : torch.Tensor, optional
        Previous denoised coordinates for old_gen, shape [B*multiplicity, N_atoms, 3]
    kernel_type : str
        Type of kernel to use: "attn_new" or "attn"
    aggregate_weights_method : str
        How to aggregate atom-level weights to sample-level: "mean" or "sum_normalized"
    **energy_loss_kwargs
        Additional arguments passed to attn_loss_new or group_contra_loss
        (e.g., R_list, target_ratio, n_sinkhorn_steps, etc.)
    
    Returns
    -------
    torch.Tensor
        Loss value, shape [B] (per-batch loss)
    dict
        Info dictionary with loss breakdown and diagnostics
    """
    B_multiplicity, N_atoms, spatial_dim = denoised_atom_coords.shape
    B = B_multiplicity // multiplicity
    
    # Verify shapes
    assert B_multiplicity == multiplicity * B, f"B_multiplicity ({B_multiplicity}) must equal multiplicity ({multiplicity}) * B"
    assert ground_truth_coords.shape == denoised_atom_coords.shape
    assert spatial_dim == 3, "Expected 3D coordinates"
    
    # Reshape coordinates from [B*multiplicity, N_atoms, 3] to [B, multiplicity, N_atoms*3]
    # This treats each multiplicity sample as a separate coordinate set for the contrastive loss.
    # Note: ground_truth_coords is already repeated multiplicity times (one copy per denoised sample),
    # so reshaping to [B, multiplicity, ...] is valid.
    denoised_flat = denoised_atom_coords.view(B, multiplicity, N_atoms * spatial_dim)  # [B, multiplicity, N_atoms*3]
    ground_truth_flat = ground_truth_coords.view(B, multiplicity, N_atoms * spatial_dim)  # [B, multiplicity, N_atoms*3]
    
    # Handle old_denoised_coords if provided
    old_gen_flat = None
    if old_denoised_coords is not None:
        assert old_denoised_coords.shape == denoised_atom_coords.shape
        old_gen_flat = old_denoised_coords.view(B, multiplicity, N_atoms * spatial_dim)
    
    # Aggregate atom-level weights to sample-level weights
    # align_weights: [B*multiplicity, N_atoms]
    # resolved_atom_mask: [B*multiplicity, N_atoms]
    
    # Combine weights with mask
    effective_weights = align_weights * resolved_atom_mask  # [B*multiplicity, N_atoms]
    
    if aggregate_weights_method == "mean":
        # Average weights across atoms for each sample
        weight_gen = effective_weights.mean(dim=-1)  # [B*multiplicity]
    elif aggregate_weights_method == "sum_normalized":
        # Sum weights normalized by number of resolved atoms
        weight_gen = effective_weights.sum(dim=-1) / (resolved_atom_mask.sum(dim=-1) + 1e-8)  # [B*multiplicity]
    else:
        raise ValueError(f"Unknown aggregate_weights_method: {aggregate_weights_method}")
    
    # Reshape weights to [B, multiplicity]
    weight_gen = weight_gen.view(B, multiplicity)  # [B, multiplicity]
    
    # For ground truth, we typically want uniform weights (all samples are equally valid)
    # But we can also use the same weights if desired
    weight_pos = torch.ones_like(weight_gen)  # [B, multiplicity]
    # Alternative: use same weights as gen
    # weight_pos = weight_gen.clone()
    
    # Call the energy loss function
    if kernel_type == "attn_new":
        # Use attn_loss_new directly
        # gen: [B, multiplicity, N_atoms*3]
        # fixed_pos: [B, multiplicity, N_atoms*3] - we use ground truth as positive
        # For attn_loss_new, we want gen to be attracted to fixed_pos
        # So gen=denoised, fixed_pos=ground_truth
        
        result = attn_loss_new(
            gen=denoised_flat,  # [B, multiplicity, N_atoms*3]
            fixed_pos=ground_truth_flat,  # [B, multiplicity, N_atoms*3]
            fixed_neg=None,  # No negative samples for now
            weight_gen=weight_gen,  # [B, multiplicity]
            weight_pos=weight_pos,  # [B, multiplicity]
            weight_neg=None,
            old_gen=old_gen_flat,  # [B, multiplicity, N_atoms*3] or None
            **energy_loss_kwargs,
        )
        # attn_loss_new always returns (loss, info)
        loss, info = result
        
    elif kernel_type == "attn":
        # Use group_contra_loss with attn kernel
        # This expects pos (positive), gen (generated), neg (negative, optional)
        result = group_contra_loss(
            pos=ground_truth_flat,  # [B, multiplicity, N_atoms*3] - positive targets
            gen=denoised_flat,  # [B, multiplicity, N_atoms*3] - generated samples
            neg=None,  # No negative samples for now
            pos_w=weight_pos,  # [B, multiplicity]
            gen_w=weight_gen,  # [B, multiplicity]
            neg_w=None,
            kernel_type="attn",
            old_gen=old_gen_flat,  # [B, multiplicity, N_atoms*3] or None
            return_info=True,
        ...
        # group_contra_loss with return_info=True returns (loss, info)
        if isinstance(result, tuple):
            loss, info = result
        else:
            # Fallback if return_info wasn't respected
            loss = result
            info = {}
    else:
        raise ValueError(f"Unknown kernel_type: {kernel_type}")
    
    # Loss shape should be [B] - per-batch loss
    # If it's already a scalar, expand it
    if loss.dim() == 0:
        loss = loss.unsqueeze(0).expand(B)
    elif loss.shape[0] != B:
        # If loss is [B*multiplicity], average over multiplicity
        if loss.shape[0] == B_multiplicity:
            loss = loss.view(B, multiplicity).mean(dim=1)
        else:
            raise ValueError(f"Unexpected loss shape: {loss.shape}, expected [B] or [B*multiplicity]")
    
    # Add sigma information to info dict
    info["sigma_mean"] = sigmas.mean().item()
    info["sigma_std"] = sigmas.std().item()
